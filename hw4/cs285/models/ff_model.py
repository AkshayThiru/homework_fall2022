...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized = utils.normalize(obs_unnormalized, obs_mean, obs_std)
        acs_normalized = utils.normalize(acs_unnormalized, acs_mean, acs_std)
        ...
        delta_pred_unnormalized = utils.unnormalize(delta_pred_normalized, delta_mean, delta_std)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self(observations, actions, *[ptu.from_numpy(v) for v in data_statistics.values()])
        prediction = ptu.to_numpy(prediction) # TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = utils.normalize(next_observations - observations, data_statistics['delta_mean'], data_statistics['delta_std'])
        target = ptu.from_numpy(target) # TODO(Q1) compute the normalized target for the model.
        ...
        _, delta_pred_normalized = self(observations, actions, *[ptu.from_numpy(v) for v in data_statistics.values()])
