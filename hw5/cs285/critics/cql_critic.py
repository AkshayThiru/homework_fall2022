...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        """ TODO: Implement DQN Loss (see dqn_critic.py) """
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        q_t_logsumexp = torch.max(qa_t_values, 1).values + torch.log(torch.sum(torch.exp(qa_t_values - torch.max(qa_t_values, 1, keepdim=True).values), 1))
