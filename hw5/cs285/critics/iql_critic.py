...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        """
        # TODO: Use the correct formula for expectile loss.
        weights = torch.where(diff >= 0.0, self.iql_expectile, np.abs(1 - self.iql_expectile))
        return (weights * torch.square(diff)).mean()
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE HERE ###
        # TODO: Use target Q function here.
        qa_t_values = self.q_net_target(ob_no)
        q_t_values = torch.gather(qa_t_values, 1, ac_na.unsqueeze(1)).squeeze(1).detach()
        v_values = self.v_net(ob_no).squeeze(1)
        assert q_t_values.shape == v_values.shape
        value_loss = self.expectile_loss(q_t_values - v_values)
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        target_values = reward_n + self.gamma * self.v_net(next_ob_no).squeeze(1).detach()
